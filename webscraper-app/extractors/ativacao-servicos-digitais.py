"""extractors/ativacao-servicos-digitais.py — Ativação de Serviços Digitais"""

from bs4 import BeautifulSoup, Tag
from extractors.base import (
    _extract_page_title_section, make_faq_duplicate_checker,
    handle_comunicados, handle_side_by_side_component, handle_side_by_side_row,
    handle_nav_links, handle_legaltext, handle_tabs_component, handle_teaser,
    handle_h2, handle_p_h2, handle_steps_standalone, handle_end_of_page, handle_p_standalone,
)
...
def extract_sections(soup: BeautifulSoup) -> list[dict]:
    ...
    def walk(node: Tag):
        ...
        if handle_comunicados(node, sections, is_faq_duplicate, use_richtext_full=False): return
        ...
        if handle_teaser(node, sections, visited_teasers, use_richtext_full=False): return
